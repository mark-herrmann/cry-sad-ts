...
* Progress update function can be passed on vault creation and unlocking (see: [Usage](#usage))
...
Both keys are encrypted using RFC 3394 key wrapping (also known as aes-key-wrap) with a KEK (key-encryption-key) derived from the password using scrypt with a 8-byte salt.
```
...
* [node-forge](https://www.npmjs.com/package/node-forge) by [digitalbazaar](https://github.com/digitalbazaar), \
  used for authenticated encryption and decryption (AES-256 and HMAC-SHA256), and for CSPRNG purposes
* [jsonwebtoken](https://www.npmjs.com/package/jsonwebtoken) by [auth0](https://github.com/auth0), \
  used for JWT purposes
* [uuid](https://www.npmjs.com/package/uuid) by [uuidjs](https://github.com/uuidjs), \
  used to create the unique vault id (stored in the JWT)
* [scrypt-js](https://www.npmjs.com/package/scrypt-js) by [ricmoo](https://github.com/ricmoo),
  used to derive the KEK from the password
* [@stablelib/siv](https://www.npmjs.com/package/@stablelib/siv) by [StableLib](https://github.com/StableLib), \
  used for SIV operation mode, required for deterministic string encryption and decryption
...
* Performance: \
  The currently used library scrypt is a relatively slow implementation. \
  It will take a few seconds, to derive the KEK. \
  There are faster implementations, but currently they can not be used, since they do not (properly) support browser environments. \
  Since, the KEK derivation usally not happens that frequently, this seems to be an acceptable performance issue. \
  It would be much less secure, if we would use a smaller cost param for scrypt, to increase the speed.
* Campatibility: \
  crypto-js uses the random methods of the WebCrypto API. \
  For this reason cry-sad-ts might not run in some JavaScript environments without WebCrypto API. Such as IE 10 or before or React Native.
